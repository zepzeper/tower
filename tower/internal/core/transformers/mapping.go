...
// DataMap represents a generic data structure used for transformations
...
func (t *Transformer) Transform(input DataMap) (DataMap, error) {
...
// getNestedValue retrieves a potentially nested value from a DataMap
func getNestedValue(data DataMap, path string) (interface{}, error) {
	...
	for i := 0; i < len(parts)-1; i++ {
		...
		// Try to convert to DataMap
		nextMap, ok := next.(map[string]interface{})
		if !ok {
			// Try another type of map
			nextMapAlt, ok := next.(DataMap)
			if !ok {
		...
		// Convert to DataMap for consistency
		current = DataMap(nextMap)
	}
...
// setNestedValue sets a potentially nested value in a DataMap
func setNestedValue(data DataMap, path string, value interface{}) error {
	parts := strings.Split(path, ".")
	current := data
	...
	for i := 0; i < len(parts)-1; i++ {
		...
		// Try to convert existing value to DataMap
		nextMap, ok := next.(map[string]interface{})
		if !ok {
			// Try another type of map
			nextMapAlt, ok := next.(DataMap)
