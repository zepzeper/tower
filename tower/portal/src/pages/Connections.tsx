...
const Connections: React.FC = () => {
  ...
  const [isActiveApiModalOpen, setActiveApiModalOpen] = useState<boolean>(false);
  ...
  const getStatusDisplay = (status: Connection['status']): StatusDisplay => {
    switch (status) {
      case 'active':
        return {
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                    {connection.lastSync ? (
                      <>
                        <span className="font-medium">{t('connections.lastSync')}:</span>{' '}
              ...
              <div className="mt-4 pt-4 flex justify-end gap-2">
                ...
                {status.action === 'view' ? (
                  <Link to={`${status.route}${connection.id}`}>
                    <button className="px-4 py-2 text-sm font-medium rounded-md bg-green-100 text-green-800 hover:bg-green-200 dark:bg-green-900 dark:bg-opacity-30 dark:text-green-300 dark:hover:bg-opacity-40">
                      {status.buttonLabel}
                    </button>
                  </Link>
                ) : (
